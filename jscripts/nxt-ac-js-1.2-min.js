...
(function (a) {
	a.widget("ui.autocomplete", {
		options: {
			...
			productsHeader: '<div class="productsHeader">PRODUCT MATCHES</div>',
			itemOrder: ["terms", "products","brands","categories"],
			footer: '<div style="text-align:center;border-top:1px solid black;"><a target="_blank" href="http://www.nextopia.com"><img border="0" src="//d2brc35ftammet.cloudfront.net/autocomplete/autocomplete-powered-by-nextopiav1.gif"></a></div>'
		},
		_create: function () {
			var b = this,
			...
			//makes sure selection events don't apply to section labels
			...
			var languageCode = $('body').data('language').toUpperCase();

			this.source = function (e, d) {
			
				var extraComplete = false;
				var extraResults = new Object();
				//only stress out local DB after 3 characters
				if(e.term.length >= 3 || languageCode != 'EN') {
					a.ajax({
						url: "search.aspx/GetAutocompleteBrandsAndDepts",
						data: JSON.stringify({ keywords: e.term, languageCode: languageCode }),
						type: "POST",
						...
						success: function (data) {
							...
							if (languageCode != 'EN') {
							    ...
							    for (var prop in extraResults) {
							        if (extraResults[prop].length > 0 || prop == 'Translations') {
							            result[prop.toLocaleLowerCase()] = extraResults[prop]
							        }
							    }
							    d(result);
							}
				...
				a.ajax({
					...
					success: function (f) {
							...
							//append extra local DB results to nextopia ones
							var limit = 0;
							//sometimes nextopia returns nothing (legitametly. I think)
				                if (f == undefined) {
								f = new Object();
							}
							//using a loop to wait 100ms if necessary (in reality local db should get back before nextopia)
				                var interval = setInterval(function () {
								if (extraComplete || limit > 10) {
									clearInterval(interval);
									for (var prop in extraResults) {
				                            if (prop != 'Terms' && extraResults[prop].length > 0) {
				                                f[prop.toLocaleLowerCase()] = extraResults[prop]
									}
				                        }
									d(f);
				                    } else {
			...
			//this.menu.element.wrap('<div class="nxt-autocomplete-wrapper"/>');
		},
		destroy: function () {
		...
		_suggest: function (items) {
			var ul = this.menu.element.empty().zIndex(this._getZIndex() + 1),
                d, e;
...
//			ul.width(Math.max(d, e));
			ul.addClass('nxt-autocomplete');
			//nice idea below to create nested list, but unfortunately breaks keyboard nav
/*			var renderedItems = ul.find('li:not(.ui-autocomplete-category)').detach();
			ul.find('li').each(function(index){
				var li = a(this);
				li.addClass("parent-suggestions");
				li.append('<ul class="child-suggestions"></ul>');
				var section = li.attr('data-section');
				li.find('ul.child-suggestions"').append(renderedItems.filter('[data-section="'+section+'"]'));
			});*/
			//ul.empty();
			this._positionMenu(ul);

		},
		_positionMenu: function (ul) {
			ul.position({
				my: "left top",
				at: "left bottom",
				of: this.element,
...
//			ul.parent().width(ul.outerWidth()).height(ul.outerHeight());
//			ul.parent().position({
//				my: "left top",
//				at: "left bottom",
//				of: this.element,
...
//			}).show();
		...
		_renderMenu: function (ul, items) {
			...
			for (var d in b.options.itemOrder) {
				var itemType = b.options.itemOrder[d];
				if (items[itemType] !== undefined) {
			        if (items[itemType].length > 0) {
			            var sectionTitle = ((itemType == 'terms') ? 'suggestions' : itemType);
			            if (items.translations != undefined && items.translations[sectionTitle] != undefined) {
			                sectionTitle = items.translations[sectionTitle];
			            }
			            ul.append('<li class="ui-autocomplete-category" data-section="' + itemType + '"><b>' + sectionTitle + "</b></li>");
					a.each(items[itemType], function (index, item) {
			                item['translations'] = items.translations;
						b._renderItem(ul, item, itemType)
						//b._renderItem(subList, item, itemType)
					})
			            //					var li = a('<li>'+itemType+'</li>')
			            //					a(subList).appendTo(li);
			            //					li.appendTo(ul);
				}
			}
			}
//			if (b.options.footer != "") {
//				a(b.options.footer).appendTo(ul)
//			}

		},
		_renderItem: function (ul, dataItem, itemType) {
			var itemToDisplay = "";
			switch (itemType) {
				case "terms":
					itemToDisplay = this._drawTerm(dataItem);
					break;
				case "products":
					itemToDisplay = this._drawProduct(dataItem);
					break
				case "categories":
					itemToDisplay = this._drawDept(dataItem);
					break
				case "brands":
					itemToDisplay = this._drawBrand(dataItem);
					break
			}
			return a('<li data-section="'+itemType+'"></li>').data("item.autocomplete", dataItem).addClass(itemType + "Item").append("<a>" + itemToDisplay + "</a>").appendTo(ul)
		},
		...
		_drawProduct: function (b) {
			...
			return '<img src="' + b.Image + '"/><span class="nxt-ac-item-text"><p>' + this._encapsulateMatches(b.Name) + '</p><span class="nxt-ac-item-price">$' + Number(b.Price).toFixed(2) + '</span></span>'
		},
		_drawBrand: function (b) {

			return this._encapsulateMatches(b.Name);
		},
		_drawDept: function (b) {
			var text = this._drawBrand(b);
			if (b.ParentName != undefined && b.ParentName.length > 0) {
			    if (b.translations != undefined && b.translations['in'] != undefined) {
			        text += ' ' + b.translations['in'] + ' ';
			    } else {
			        text += ' in ';
			}
				text += a('<i></i>').text(b.ParentName)[0].outerHTML;
			}
			return text;
		},
